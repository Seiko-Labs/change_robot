...
from config import *
from pyautogui import locateCenterOnScreen, click, moveTo, locateOnScreen, locateAllOnScreen
...
class Colvir:
    ...
    def colvir_is_card_exist(self) -> bool:
        # self.colvir_start()
    ...
    def colvir_add_permissions(self):
        ...
        def create_position():
            ...
            pythonRPA.keyboard.press('TAB')
            sleep(1)
            df = pd.read_excel('glossary.xlsx', sheet_name='AWG')
            codes = df['Code']
            groups = df['Workgroup']
            awg = None
        ...
        def unlink_additional_position():
            ...
            pythonRPA.keyboard.write(self.data['user'], timing_before=1, timing_between=0.3)
            sleep(1)
            selector.click()
            sleep(1)
    ...
    def is_user_exist(self, num, selector):
        ...
        if pythonRPA.bySelector([{'class_name': 'TMessageForm', 'title': 'Информация', 'backend': 'uia'}]).is_exists():
            ...
            num += 1
            # self.data['user'] += self.text[1][num]
            # pythonRPA.keyboard.write(self.text[1][num])
    ...
    def change_role(self):
        self.colvir_start()
        sleep(2)
        df = pd.read_excel('glossary.xlsx', sheet_name='USRGRANT')
        codes = df['Code']
        roles = df['Role']
        departments = df['Department']
        ...
        click(locateCenterOnScreen(
            ...
            confidence=0.7))
        sleep(1)
        pythonRPA.keyboard.write(self.data['user'], timing_between=0.05)  # Логин пользователя
        selector.click()
        ...
        pythonRPA.keyboard.press('F4')
        pythonRPA.bySelector(
            [{'class_name': 'TfrmUsrGrantGrpDetail', 'title': f'Позиция {self.data["user"]}', 'backend': 'uia'},
             {'class_name': 'TExPanel', 'control_type': 'Pane', 'title': ''},
             {'class_name': 'TExPanel', 'control_type': 'Pane', 'title': ''},
             {'class_name': 'TExPanel', 'control_type': 'Pane', 'title': ''}]).wait_appear(self.default_wait)
        pythonRPA.keyboard.press('TAB')
        pythonRPA.keyboard.write(awg, timing_before=0.05)
        pythonRPA.keyboard.press('ENTER', timing_after=1)
        pythonRPA.keyboard.press('PAGE_DOWN', timing_after=1)
        sleep(2)
        ...
        for i in range(10):
            click(locateCenterOnScreen(
                r"C:\Users\robot.ad\Desktop\UserCreationRobot\Sources\img\colvir\delete_role.png"))
            sleep(0.5)
        pythonRPA.bySelector([{'class_name': 'TfrmHieFrmOrg',
                               'title': f'Зависимости - Пользователи - {self.data["username"]}',
                               'backend': 'uia'}, {'class_name': 'TExPanel', 'control_type': 'Pane', 'title': ''},
                              {'class_name': 'TPanel', 'control_type': 'Pane', 'title': 'pnlGrid'},
                              {'class_name': 'TPanel', 'control_type': 'Pane', 'title': ''},
                              {'class_name': 'TPanel', 'control_type': 'Pane', 'title': ''}]).click()
        sleep(1)
        ...
        for role in roles_to_append:
            ...
            pythonRPA.keyboard.press('ENTER')
            sleep(3)
            if locateOnScreen(r'C:\Users\robot.ad\Desktop\UserCreationRobot_COPY\Sources\img\colvir\nodata.png'):
                print('Профиль полномочий не найден или уже добавлен')
                pythonRPA.bySelector([{'class_name': 'TfrmHieFrmOrg',
                                       'title': f'Зависимости - Пользователи - {self.data["username"]}',
                                       'backend': 'uia'},
                                      {'class_name': 'TExPanel', 'control_type': 'Pane', 'title': ''},
